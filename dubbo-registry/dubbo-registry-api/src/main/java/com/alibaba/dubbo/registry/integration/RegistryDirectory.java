...
public class RegistryDirectory<T> extends AbstractDirectory<T> implements NotifyListener {
    ...
    public void destroy() {
        ...
        try {
            if(getConsumerUrl() != null && registry != null && registry.isAvailable()) {
                registry.unregister(getConsumerUrl().removeParameter("category").addParameters(new String[]{"category", "consumers", "check", String.valueOf(false)}));
            }
        } catch (Throwable t) {
            logger.warn("unexpeced error when unregister service " + serviceKey + "from registry" + registry.getUrl(), t);
        }
